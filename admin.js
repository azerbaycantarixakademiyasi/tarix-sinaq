...
window.openNewQuizForm = function() {
    let h = `
        <div style="background:#f9f9f9; padding:15px; border-radius:10px;">
            ...
            <input id="q-title" placeholder="Sınağın Başlığı (məs: Ümumi Tarix #1)">
            <div id="dim-questions-container" style="max-height:400px; overflow-y:auto; margin:15px 0; border:1px solid #ddd; padding:10px;">
                <p style="color:red; font-size:12px;">Doldurmaq üçün aşağıdakı düyməni basın.</p>
            </div>
            <button onclick="generateDIMFields()" style="background:#3498db;">Sual Sahələrini Yarat (1-30)</button>
...
window.generateDIMFields = function() {
    ...
    container.innerHTML = "";
    for(let i=1; i<=30; i++) {
        let typeInfo = i <= 22 ? "Qapalı" : (i <= 26 ? "Rəqəmli" : (i == 27 ? "Uyğunluq" : "Açıq/Situasiya"));
        container.innerHTML += `
            <div class="question-box" style="margin-bottom:20px; border-left:4px solid #1a4e8a; padding-left:10px;">
                <b>Sual ${i} (${typeInfo})</b>
                <input id="q-txt-${i}" placeholder="Sual mətni">
                ${i <= 22 ? `
                ...
                <input id="q-cor-${i}" placeholder="Düzgün Cavab (Məs: A və ya 2,3 və ya 1-a,2-b)" style="background:#e8f5e9;">
            </div>`;
    }
};
...
window.saveDIMQuiz = function() {
    ...
    if(!title) return alert("Sınaq adını yazın!");
    
    let questions = [];
    for(let i=1; i<=30; i++) {
        let qData = {
            text: document.getElementById(`q-txt-${i}`).value,
            correct: document.getElementById(`q-cor-${i}`).value.trim()
        };
        if(i <= 22) {
            qData.variants = {
    ...
    db.ref('quizzes').push({ title: title, active: false, questions: questions })
    .then(() => { alert("Sınaq uğurla yaradıldı!"); window.admTab('quiz-sec'); });
};

function loadAdminQuizzes() {
    db.ref('quizzes').on('value', snap => {
        let h = "<h3>Sınaqlar</h3>";
        snap.forEach(c => {
            const q = c.val();
...
// 6. Nəticələr və Materiallar
function loadAdminResults() {
    db.ref('results').on('value', snap => {
        let h = "<table><tr><th>Şagird</th><th>Sınaq</th><th>Düz</th><th>Sil</th></tr>";
        snap.forEach(c => {
            const r = c.val();
...
window.addM = function() {
    ...
    if (t && l) db.ref('materials').push({ title: t, link: l }).then(() => alert("Material yükləndi!"));
};
