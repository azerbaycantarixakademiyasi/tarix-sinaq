...
function loadResults() {
    database.ref('results').on('value', snap => {
        let h = `<table><tr><th>Şagird</th><th>Bal</th><th>Tarix</th><th>Bax</th></tr>`;
        snap.forEach(c => {
            const r = c.val();
            h += `<tr><td>${r.studentName}</td><td>${r.score}/${r.maxScore}</td><td style="font-size:10px;">${r.date}</td><td><button onclick="viewDetail('${c.key}')" style="width:auto; padding:2px 8px;">👁</button></td></tr>`;
        });
...
window.viewDetail = (id) => {
    database.ref('results/' + id).once('value', snap => {
        ...
        // ... detallı baxış kodları ...
        document.getElementById('modal-content').innerHTML = c + "Detallar üçün nəticələr bazasına baxın.";
